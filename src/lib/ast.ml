...
type identifier = string * position [@@deriving show]

(* The kind of a literal expression. *)
type literal_kind = LitInt of int | LitString of string [@@deriving show]

(* A literal expression is a literal with its position in the source code. *)
type literal = literal_kind * position [@@deriving show]
...
and pattern = pattern_kind * position [@@deriving show]

(* The kind of statement in the language. *)
type sttm_kind = SttmLet of pattern * expr | SttmExpr of expr
[@@deriving show]

(* A statement consists of its kind and the associated expression. *)
and sttm = sttm_kind * position [@@deriving show]
...
and expr = expr_kind * position [@@deriving show]
...
type binder_kind = identifier * expr option [@@deriving show]

(* A binder consists of its kind and its position in the source code. *)
type binder = binder_kind * position [@@deriving show]
...
type declaration_value = TypeDef of type_decl | LetDecl of let_decl
[@@deriving show]

(* The visibility of a declaration, which can be either public or private. *)
type declaration_visibility = Public | Private [@@deriving show]

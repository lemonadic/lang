...
let print_compiler_error fmt error source =
  let pp_range fmt { start_pos; end_pos } =
    Format.fprintf fmt "%d:%d~%d:%d" start_pos.line start_pos.column
      end_pos.line end_pos.column
  in

  Format.fprintf fmt "\n %s%sERROR E%04d%s: %s%s\n" bold red error.id white
    error.message reset;
  ...
  let relevant_lines =
    expand_positions
      (error.location :: List.map (fun (_, pos) -> pos) error.hints)
  in
    ...
    (fun line_num ->
      if line_num > 0 && line_num <= Array.length source_lines then (
        ...
        if line_num = error.location.start_pos.line then
          ...
          let caret_line =
            String.make start_col ' ' ^ String.make error_length '^'
          in
